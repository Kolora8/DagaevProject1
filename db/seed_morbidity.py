...
def main():
    ...
    for year, diseases in data["rf"]["morbidity"].items():
        for dkey, val in diseases.items():
            cur.execute(
                "INSERT INTO morbidity (region_code, year, disease_key, absolute_numbers, per_100000) VALUES (%s, %s, %s, %s, %s) ON CONFLICT DO NOTHING",
    ...
    for r in data["regions"]:
        ...
        for year, val in r.get("water_quality", {}).items():
            # Skip non-year keys like "parameters" that hold chemical parameter metadata
            if not isinstance(val, dict) or "safe_water_pct" not in val:
                continue
            cur.execute(
                "INSERT INTO water_quality_data (region_code, year, safe_water_pct, chem_violation_pct, micro_violation_pct, pipe_violation_pct) VALUES (%s, %s, %s, %s, %s, %s) ON CONFLICT DO NOTHING",
        ...
        for year, val in r.get("emissions", {}).items():
            cur.execute(
                "INSERT INTO emissions_data (region_code, year, total_kt, per_capita_kg, stationary_kt, mobile_kt) VALUES (%s, %s, %s, %s, %s, %s) ON CONFLICT DO NOTHING",
